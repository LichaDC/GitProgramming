...
def changeNamePhotos(path, file):
    ...
    while True:
        ...
        if not os.path.isfile(correctName): # Check if doesn't exists a file with the correct name.
            ...
            if newer(correctName, pathFile): # Check if it's older than the correct named file. (correctName is newer)
                j = 1
                while True:
                    if not os.path.isfile(os.path.join(path, '{}-{}.JPG'.format(eval(schemeP)[:-4], j))): # Check if the temporal name doesn't exist
                        print('[ → ] - I renamed', eval(schemeP), '       by        {}-{}.JPG'.format(eval(schemeP)[:-4], j))
                        os.replace(correctName, os.path.join(path, '{}-{}.JPG'.format(eval(schemeP)[:-4], j))) # Give temporal name to the correct named file
                        global unordered
                        unordered = True
                        break
                    j += 1
...
def changeNameVideos(path, file):
    pathFile = os.path.join(path, file)
    d = date(pathFile)
    i = 1
    while True:
        correctName = os.path.join(path, eval(schemeV))
        if not os.path.isfile(correctName): # Check if doesn't exists a file with the correct name.
            print('[ → ] - I renamed', file, '       by       ', eval(schemeV))
            os.replace(pathFile, correctName)
            break
        elif correctName == pathFile:
            pass
            ...
            if newer(correctName, pathFile): # Check if it's older than the correct named file. (correctName is newer)
                j = 1
                while True:
                    if not os.path.isfile(os.path.join(path, '{}-{}.MPG'.format(eval(schemeV)[:-4], j))): # Check if the temporal name doesn't exist
                        print('[ → ] - I renamed', eval(schemeV), '       by        {}-{}.MPG'.format(eval(schemeV)[:-4], j))
                        os.replace(correctName, os.path.join(path, '{}-{}.MPG'.format(eval(schemeV)[:-4], j))) # Give temporal name to the correct named file
                        global unordered
                        unordered = True
                        break
                    j += 1
    ...
    for dirpath, dirnames, filenames in os.walk(directory):
        for dir in filenames:
            if dir[-4:] == '.MPG' or dir[-4:] == '.mpg':
                if not unordered:
                    changeNameVideos(dirpath, dir)
print()
print("| ----- - -----[     Every video has the correct name     ]----- - ----- |")
